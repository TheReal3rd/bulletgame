...
def collision():
    global PlayerOne, EnemyOne, enemyHealth, screenFlash
    #Enemy Bullets
    for value in sprites.all_of_kind(SpriteKind.EnemyProjectile):
        ...
        if dist >= scene.screen_width():## Off screen remove it.
...
def updateEnemy():
    ...
    if EnemyStage == 0:
        if enemyFireDelay.passed(2000):## if 2000 ms passed the enemy will shoot.
            angle = 60
            if(Math.percent_chance(50)):
                angle = -60
            shootBullets(EnemyOne.x, EnemyOne.y, angle, 0, 4)
            enemyFireDelay.reset()
        elif Math.percent_chance(2):## 2% chance to generate a new waypoint.
            if waypoint == None:
    ...
    if waypoint != None:## IF we have a waypoint we move towards it.
        #X
...
def shootBullets(posX2: number, posY2: number, angleOffset: number, typeBullet: number, numBullets: number):
